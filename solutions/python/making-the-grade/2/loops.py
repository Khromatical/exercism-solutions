...
def count_failed_students(student_scores):
    ...
    """
    non_passing_students = 0
    for score in student_scores:
        if score <= 40:
            non_passing_students += 1
    return non_passing_students
...
def letter_grades(highest):
    ...
    delta = (highest - 40) // 4
    return [delta * number + 41 for number in range(4)]
...
def student_ranking(student_scores, student_names):
    ...
    for index, word in enumerate(student_names):
        rank = str(index + 1)
        name = str(word)
        score = str(student_scores[index])
        new_string = f'{rank}. {name}: {score}'
        ranked_scores.append(new_string)
