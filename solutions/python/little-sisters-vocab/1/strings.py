...
def make_word_groups(vocab_words):
    ...
    words = vocab_words[1::]
    separator = f' :: {prefix}'
    prefixed_first_word = f'{prefix} :: {prefix}'
    prefixed_other_words = separator.join(words)
    return f'{prefixed_first_word}{prefixed_other_words}'
...
def remove_suffix_ness(word):
    ...
    final_word = ''
    remove_suffix = word.replace('ness', '')
    if 'i' in remove_suffix[-1]:
        final_word = remove_suffix.replace('i', 'y')
    else:
        final_word = remove_suffix
    return final_word
...
def adjective_to_verb(sentence, index):
    ...
    """
    remove_period = sentence.replace('.', '')
    split_sentence = remove_period.split()
    adjective = split_sentence[index]
    make_verb = adjective + 'en'
    return make_verb
